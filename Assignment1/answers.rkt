...
(define (area [shape : Shape]) : Number
  (local
      [(define (*/check a b)
         (if (and (>= a 0) (>= b 0))
             (* a b)
             (error 'area "Negative measurement")))]
    (type-case Shape shape
      [(Square side) (*/check side side)]
      [(Circle radius) (* pi (*/check radius radius))]
      [(Triangle height width) (/ (*/check  height width) 2)])))
...
(define (tern->num digits)
  (local [(define (first-digit)
            ;; The main purpose of this function is error checking.
            ;; Note the use of lexical-scope (reference to digits in local function)
            (let ([fst (first digits)])
              (case fst
                [(0 1 2) fst]
                [else (error 'tern->num (string-append "bad digit: " (to-string fst)))])))]
        (cond
          [(empty? digits) 0]
          [else (+ (first-digit) (* 3 (tern->num (rest digits))))])))
